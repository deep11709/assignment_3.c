...
{
  int r, max = 0; // variable r to store random no. and max_base for greatest digit of random no.
  while (n > 0)        // loop will work till the random no. become 0
  {
    r = n % 10;       // remainder of no. is stored every time loop works
    if (r > max) // to check greatest digit
    {
      max = r; // greatest digit is stored here after comparision
    }
    n = n / 10; // n will get shorten ever time loop works
  }

  return max + 1; // retrns the base of random no. as base is one more than greatest digit
}


int main()
{
  long long int random[50]; 
  ...
  {
    random[i] = rand() % 100000000 + 5000000000; // random numbers stored in array
    large[i] = base(random[i]);         // base function is called

    printf(" The Highest Base of %lld is %d \n", random[i], large[i]); // prints random number and base
  }
